...
export const addAction = async (req, res) => {
  let { title, price, priceneg, desc, cat, token } = req.query;
  ...
  if (req.files && req.files.img) {
    if (req.files.img.length == undefined) {
      ...
      for (var i = 0; i < req.files.img.data; i++) {
        if (
          ["image/jpeg", "image/jpg", "image/png"].includes(
...
export const getList = async (req, res) => {
  ...
  if (state) {
    ...
    if (s) {
      filters.category = s._id.toString();
    }
  }
...
export const editAction = async (req, res) => {
  let { id } = req.params;
  let { title, status, price, priceneg, desc, cat, images, token } = req.query;
